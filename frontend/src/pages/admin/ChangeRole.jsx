...
function ChangeRole() {
    ...
    const handleGetUser = async () => {
        ...
        try {
            ...
            const filteredUsers = response.data.data
                .filter(user => !user.authorities.some(auth => auth.authority === "ROLE_SUDO"))
                .map(user => ({
                    ...user,
                    roles: user.authorities.map(auth => auth.authority).length > 0 ? user.authorities.map(auth => auth.authority) : ['Usuario']
                }));
    ...
    const handleChangeRole = async (username, newRole) => {
        ...
        try {
            const response = await axios.post("http://localhost:8080/api/config/change-roles", {
                identifier: username,
                roles: [newRole]
            }, {
                headers: { 'Authorization': `Bearer ${token}` }
                ...
                <div className="user-info-container">
                    ...
                    {users.map((user, index) => (
                        <div key={index} className="users-info">
                            ...
                            <div>
                                <select defaultValue="" onChange={(e) => handleChangeRole(user.username, e.target.value)}>
